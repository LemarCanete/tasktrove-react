...
const GanttChart: React.FC<{ id: string }> = ({id}) => {
    ...
    useEffect(() => {
        const fetchData = async (): Promise<void> => {
          try {
            ...
            teams.data.forEach((team: any) => teamIds.push(team._id));
            ...
            const user_tasks = tasks.data.filter((task: any) => teamIds.includes(task.team_id));
      
            // Group tasks under their respective projects
            const data: any[] = [];
      
            projects.data.forEach((project: any) => {
              const projectTasks = user_tasks.filter((task: any) => task.project_id === project._id);
